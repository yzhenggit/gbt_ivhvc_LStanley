...
ld_cube = 'data/negative_velo_cube_Stanley.fits'
...
fig.tight_layout()
fig.savefig('figs/LD_GBT_cloud_spec/%s_LD_GBT_RA%.2f_DEC%.2f_vchan_nomw.pdf'%(point_tag, pra, pdec))
plt.close()
...
plt.plot(ld_vlsr,ld_spec, color='r', label='GBT/LD, no mw')
plt.title('GBT vs HI4PI, RA=%.2f, DEC=%.2f'%(pra, pdec))
plt.legend(fontsize=14)
plt.xlim(-500, 200)
plt.xlabel('Velocity (km/s)')
plt.ylabel('Tb (km/s)')
plt.savefig('figs/LD_GBT_cloud_spec/%s_LD_GBT_RA%.2f_DEC%.2f_wHI4PI_nomw.pdf'%(point_tag, pra, pdec))

if point_tag == 'M33':
    plt.ylim(-0.1, 3)
else:
    plt.ylim(-0.1, 0.5)
plt.savefig('figs/LD_GBT_cloud_spec/%s_LD_GBT_RA%.2f_DEC%.2f_wHI4PI_zoom_nomw.pdf'%(point_tag, pra, pdec))
plt.close()
